...
import { isReplyDraft } from "@/lib/drafts";
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-4">
          ...
          {task.actionIssues?.length ? <div className="mt-3 space-y-1 rounded-xl p-3 text-[13px]" role="status" style={{ background: "var(--bg-alt)", color: "var(--text-2)" }}>
            {task.actionIssues.map(issue => <p key={issue}>{issue}</p>)}
          </div> : null}
          {/* The buttons that finish the job ------------------------------ */}
          <div className="mt-4 space-y-2.5">
            {primaryDraft || sourceLink ? (
              <OpenButton
                label={primaryDraft ? `Open draft in ${primaryDraft.providerLabel}` : sourceLink!.label}
                target={primaryDraft ?? sourceLink!}
            ...
            {task.draft ? (
              <div className="space-y-2">
                {task.draft.ready && !primaryDraft ? <OpenButton
                  label={task.draft.to ? `Open draft to ${task.draft.to}` : `Open draft in ${task.draft.providerLabel}`}
                  target={task.draft} preference={linkPreference} variant="draft"
                  hint="Saved in your mailbox. Review it, then send."
                /> : null}
                {primaryDraft && sourceLink ? <OpenButton label="View original email" target={sourceLink} preference={linkPreference} variant="secondary" /> : null}
                {!task.draft.ready ? <p className="text-[13px]" style={{ color: "var(--text-3)" }}>Suggested response — not yet verified in your mailbox.</p> : null}
                {task.draft.body ? (
                  <div>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      {showDraft ? "Hide response" : "Preview response"}
                    </button>
                    {showDraft ? (
                      <div
          ...
          {menu === "delegate" ? (
            <MenuCard title="Hand it to">
              <DelegatePanel task={task} team={team} preference={linkPreference} onDelegate={onDelegate} />
            </MenuCard>
          ) : null}
        </div>
