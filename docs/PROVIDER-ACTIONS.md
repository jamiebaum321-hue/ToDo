...
For users who also sync Gmail in Apple Mail, iPhone task details offer **Open original in Apple Mail** when the source has a valid RFC Message-ID. The separate `message://` URL contains that header, enclosed in encoded angle brackets; it never contains a Gmail API message/thread/draft id. It is generated during serialization and does not change Gmail's existing source, saved-draft, or delegation links. The mailbox and message must be available in Apple Mail. This source-only option does not promise to open the saved reply editor. The format is described in [first-hand Mail URL testing](https://daringfireball.net/2007/12/message_urls_leopard_mail) and [iOS testing](https://www.macstories.net/tutorials/ios-7-and-mail-message-urls/); those historical observations are not verification on the user's current iPhone. A physical device check remains required.

The Apple Mail message URL does not select an account. If the same message was delivered to multiple configured mailboxes, Mail decides which local copy to show. Keep Gmail's account-qualified saved-draft action available; do not reinterpret a successful source-message launch as proof that the intended mailbox's draft or sender was selected.
